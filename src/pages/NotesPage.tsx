...
import { getNotes, addNote } from "@/lib/store";
...
export default function NotesPage() {
  ...
  const [notes, setNotes] = useState(getNotes(profile?.id || ""));
  ...
  const handleDownload = (note: typeof notes[0]) => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `${note.title.replace(/\s+/g, "-")}.txt`; a.click();
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4 mr-1" /> New Note</Button>
      </div>

      {showCreate && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-card p-6 rounded-xl card-shadow space-y-4">
          ...
          <select value={subject} onChange={e => setSubject(e.target.value)} className="h-9 px-3 rounded-md border border-input bg-background text-sm">
            <option>Programming</option><option>Science</option><option>Math</option><option>Design</option>
          </select>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {notes.map((n, i) => (
          <motion.div key={n.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card p-5 rounded-xl card-shadow group">
            <div className="flex items-start justify-between mb-3">
              <div className="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center text-primary"><FileText className="w-5 h-5" /></div>
              <button onClick={() => handleDownload(n)} className="opacity-0 group-hover:opacity-100 transition p-1.5 rounded-md hover:bg-muted"><Download className="w-4 h-4 text-muted-foreground" /></button>
            </div>
